...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!selected_replikas || selected_replikas.length == 0) {
            ...
            const user_id = selected_replikas[0].user_id;
            await interaction.channel.send('Please wait, trying to log in...');
            const new_rep = new ReplikaInstance(user_id, channel);
            const conn_res = await new_rep.connect();
            switch (conn_res) {
                case 1:
                    await interaction.channel.send('There was an error connecting to the database. Please try again later.');
                    return;
                case 2:
                ...
                case 3:
                    ...
                    return;
                case 4:
                    await interaction.channel.send('Replika is already active.');
                    return;
            }
